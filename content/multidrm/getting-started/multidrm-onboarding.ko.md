...
```mermaid
graph TD;
  start_onboarding(멀티DRM 온보딩 시작 &nbsp) --> content_packaging["1단계 - 콘텐츠 패키징 &nbsp"]

  content_packaging --> vod_packager{"파일 기반 패키저 선택  &nbsp<br>(VOD 콘텐츠의 경우) &nbsp"}
  vod_packager --> cli_packager(PallyCon<br>CLI 패키져 &nbsp)
  vod_packager --> mediaconvert(AWS Elemental<br>MediaConvert)
  vod_packager --> bitmovin_vod(Bitmovin<br>인코더&nbsp)
  vod_packager --> wowza_vod(Wowza Streaming<br>Engine)

  cli_packager --> live_packager{"라이브 패키져 선택<br>(라이브 콘텐츠의 경우) &nbsp"}
  ...
  live_packager --> bitmovin_live(Bitmovin<br>인코더&nbsp)
  live_packager --> cpix(그 외 CPIX<br>연동 솔루션 &nbsp)
  
  wowza_live --> packaging_done[패키징 결과물 CDN 적용 &nbsp<br>- 1단계 완료]
...
```mermaid
graph TD;
  drm_license[2단계 - 서버 측 DRM 연동&nbsp] --> apple_support{FairPlay DRM을 통한 <br>애플 기기 지원 필요?&nbsp}
  apple_support -->|예&nbsp| fps_cert(FairPlay 인증서 신청 및 등록 &nbsp&nbsp)
  apple_support -->|아니오&nbsp| license_token(라이선스 토큰 생성 로직 구현 &nbsp&nbsp)
  fps_cert --> license_token
  license_token --> license_test[라이선스 토큰 연동 테스트 &nbsp&nbsp<br>- 2단계 완료]
...
```mermaid
graph TD;
  drm_client[3단계 - 클라이언트 연동 &nbsp] --> web_client{"HTML5 Player 선택<br>(웹 브라우저 지원 시) &nbsp"}
  ...
  web_client --> html5_player(기타 각종&nbsp<br>HTML5 Player)

  shaka_player --> mobile_sdk{"모바일 SDK 또는 &nbsp<br>직접 연동 선택&nbsp<br>(모바일 앱 지원 시) &nbsp"}
  ...
  mobile_sdk --> multidrm_native(멀티DRM<br>네이티브 연동 &nbsp)

  widevine_android --> ott_devices{"OTT SDK 선택&nbsp<br>(OTT 기기 지원 시) &nbsp"}
  fps_ios --> ott_devices
  ncg_android --> ott_devices
  ncg_ios --> ott_devices
  multidrm_native --> ott_devices
  ott_devices -->|안드로이드TV &nbsp| widevine_androidtv(Widevine<br>AndroidTV SDK)
  ott_devices -->|애플TV&nbsp| fps_tvos(FPS<br>tvOS SDK)
  ott_devices -->|XBox| playready_uwp(PlayReady<br>UWP SDK)
  ott_devices -->|크롬캐스트 &nbsp| chromecast(ChromeCast<br>연동&nbsp)

  widevine_androidtv --> playback_test[최종 재생 테스트 &nbsp<br>- 3단계 완료&nbsp]
  fps_tvos --> playback_test
  playready_uwp --> playback_test
  chromecast --> playback_test

  playback_test --> finish_onboarding(멀티DRM 온보딩 완료&nbsp)
